(function (global, factory) {
  ...
  function finishNode(context, start, end, node) {
      if (context & 2) {
          node.start = start;
          node.end = end;
      }
      ...
      switch (state.token) {
          ...
          case 131084: {
              ...
              while (state.token & 274432) {
                  ...
                  specifiers.push(finishNode(context, start, state.endIndex, {
  ...
  function parseTemplateSpans(state, context, tail) {
      const { startIndex: start } = state;
      return finishNode(context, start, state.endIndex, {
  ...
  function parseTemplate(state, context) {
      const { startIndex: start } = state;
      const quasis = [parseTemplateSpans(state, context, false)];
      ...
      while ((state.token = scanTemplateTail(state, context)) !== 131081) {
          quasis.push(parseTemplateSpans(state, context, false));
          expect(state, context | 32768, 131080);
          expressions.push(parseExpression(state, context));
      }
      quasis.push(parseTemplateSpans(state, context, true));
      state.assignable = state.bindable = false;
      next(state, context);
      return finishNode(context, start, state.endIndex, {
          type: 'TemplateLiteral',
  ...
  function parseObjectLiteral(state, context, scope, type) {
      ...
      while (state.token !== 536870927) {
          if (state.token === 14) {
              ...
              properties.push(finishNode(context, objStart, state.endIndex, {
                  type: 'Property',
                  key,
                  value,
