...
import { Context, Flags, ScannerState, ObjectState, Escape, RegExpState, RegexFlags, NumericState, ParenthesizedState, Expression, ArrayState } from './flags';
...
export class Parser {
        ...
        while (this.hasNext()) {
            ...
            switch (first) {
                ...
                case Chars.Hyphen:
                    {
                        this.advance();
                        if (!this.hasNext()) return Token.Subtract;
                        const next = this.nextChar();
    ...
    private peekUnicodeEscape(): number {
        ...
        if (index + 5 < this.source.length) {
            ...
            if (!isIdentifierPart(code)) {
                this.report(Errors.InvalidUnicodeEscapeSequence);
            }
                    ...
                    case Chars.UpperX:
                        {
                            ...
                            if (value < 0) this.tolerate(context, Errors.Unexpected);
                    ...
                    case Chars.UpperO:
                        {
                            ...
                            value = this.scanBinaryOrOctalDigits(state, 8, 3, Errors.Unexpected);
                            break;
                        }
                    ...
                    case Chars.UpperB:
                        {
                            ...
                            value = this.scanBinaryOrOctalDigits(state, 2, 1, Errors.Unexpected);
                            break;
                        }
            ...
            if (state & (NumericState.Decimal | NumericState.DecimalWithLeadingZero)) {
                ...
                if (isOctal) {
                    ...
                    if (context & Context.OptionsNext && this.nextChar() === Chars.Underscore) {
                        this.advance();
                        if (!this.hasNext()) this.tolerate(context, Errors.Unexpected);
                        this.flags |= Flags.ContainsSeparator;
            ...
            case Chars.UpperE:
                {
                    ...
                    if (!(ch >= Chars.Zero && ch <= Chars.Nine)) this.tolerate(context, Errors.Unexpected);
    ...
    private scanRegularExpression(context: Context): Token {
        ...
        loop:
            while (true) {
                ...
                if (!this.hasNext()) this.report(Errors.Unexpected);
            }
        ...
        loop:
            while (this.hasNext()) {
                ...
                switch (code) {
                    ...
                    case Chars.LowerG:
                        if (mask & RegexFlags.Global) this.tolerate(context, Errors.Unexpected, 'g');
                        mask |= RegexFlags.Global;
                        break;

                    case Chars.LowerI:
                        if (mask & RegexFlags.IgnoreCase) this.tolerate(context, Errors.Unexpected, 'i');
                        mask |= RegexFlags.IgnoreCase;
                        break;

                    case Chars.LowerM:
                        if (mask & RegexFlags.Multiline) this.tolerate(context, Errors.Unexpected, 'm');
                        mask |= RegexFlags.Multiline;
                        break;

                    case Chars.LowerU:
                        if (mask & RegexFlags.Unicode) this.tolerate(context, Errors.Unexpected, 'u');
                        mask |= RegexFlags.Unicode;
                        break;

                    case Chars.LowerY:
                        if (mask & RegexFlags.Sticky) this.tolerate(context, Errors.Unexpected, 'y');
                        mask |= RegexFlags.Sticky;
                        break;

                        // Stage 3 proposal
                    case Chars.LowerS:
                        if (mask & RegexFlags.DotAll) this.tolerate(context, Errors.Unexpected, 's');
    ...
    private scanString(context: Context, quote: number): Token {
        ...
        if (!hasEscape && ret === 'use strict') this.flags |= Flags.StrictDirective;
    ...
    private throwStringError(context: Context, code: Escape, isTemplate: boolean = false) {
        ...
        switch (code) {
            ...
            case Escape.StrictOctal:
                this.tolerate(context, isTemplate ? Errors.TemplateOctalLiteral : Errors.StrictOctalEscape);
    ...
    private expect(context: Context, t: Token, message: Errors = Errors.Unexpected): void {
        if (this.token !== t) this.tolerate(context, message, tokenDesc(this.token));
        this.nextToken(context);
    }
    ...
    private report(type: Errors, ...value: string[]): any {
        const error = createError(type, this.lastIndex, this.lastLine, this.lastColumn, ...value);
        throw error;
    }

    private tolerate(context: Context, type: Errors, ...value: string[]): any {
        const error = createError(type, this.lastIndex, this.lastLine, this.lastColumn, ...value);
        if (!(context & Context.OptionsTolerant)) throw error;
    ...
    private parseExportDefault(context: Context, pos: Location): ESTree.ExportDefaultDeclaration {
        ...
        switch (this.token) {
            ...
            default:
                ...
                if (this.token === Token.FromKeyword) this.report(Errors.UnexpectedToken, tokenDesc(this.token));
    ...
    private parseExportDeclaration(context: Context): ESTree.ExportAllDeclaration | ESTree.ExportNamedDeclaration | ESTree.ExportDefaultDeclaration {
        ...
        switch (this.token) {
            ...
            case Token.LeftBrace:
                ...
                } else if (isReserved) this.tolerate(context, Errors.Unexpected);
    ...
    private parseImportSpecifier(context: Context): ESTree.ImportSpecifier {
        ...
        if (!hasAs) {
            if (t & Token.Reserved) this.tolerate(context, Errors.Unexpected);
    ...
    private parseStatementListItem(context: Context): any {
        switch (this.token) {
            ...
            case Token.ClassKeyword:
                return this.parseClass(context & ~Context.Yield);
            case Token.LetKeyword:
                if (!this.isLexical(context)) return this.parseStatement(context);
                return this.parseVariableStatement(context | Context.Lexical | Context.AllowIn);
            case Token.ConstKeyword:
                return this.parseVariableStatement(context | Context.AllowIn | Context.Const | Context.Lexical);
        ...
        switch (this.token) {
            ...
            case Token.AsyncKeyword:
                if (this.nextTokenIsFuncKeywordOnSameLine(context)) {
                    ...
                    if (context & Context.AnnexB) this.report(Errors.AsyncFunctionInSingleStatementContext);
        ...
        if (t === Token.Colon && expr.type === 'Identifier') {
            ...
            switch (t) {
                ...
                case Token.FunctionKeyword:
                    ...
                    // Statement positions
                    if (context & Expression.Statement || this.flags & Flags.AllowContinue) {
            ...
            const body = this.parseStatement(context | Context.AnnexB);
    ...
    private parseIfStatementChild(context: Context): ESTree.Statement {
        ...
        return this.parseStatement(context | Context.AnnexB | Expression.Statement);
    }
    ...
    private parseWhileStatement(context: Context): ESTree.WhileStatement {
        ...
        const body = this.parseStatement(context | Context.ValidateEscape);
    ...
    private parseWithStatement(context: Context): ESTree.WhileStatement {
        ...
        this.expect(context, Token.LeftParen);
        const object = this.parseExpression(context | Expression.Statement | Context.AllowIn, pos);
        this.expect(context, Token.RightParen);
    ...
    private parseSwitchStatement(context: Context): ESTree.SwitchStatement {
        ...
        while (this.token !== Token.RightBrace) {
            ...
            if (clause.test === null) {
                ...
                if (seenDefault) this.tolerate(context, Errors.Unexpected);
    ...
    private parseVariableDeclarationList(context: Context): ESTree.VariableDeclarator[] {
        ...
        if (context & Context.ForStatement &&
            ...
            this.tolerate(context, Errors.ForInOfLoopMultiBindings, 'for-' + tokenDesc(this.token));
        }
        return list;
    }
    ...
    private parseVariableDeclaration(context: Context): any {
        ...
        if (t & Token.IsBindingPattern) {
            ...
            if (this.parseOptional(context, Token.Assign)) {
                ...
                if (context & Context.ForStatement) {
                    if (this.token === Token.OfKeyword) this.tolerate(context, Errors.InvalidVarInitForOf);
    ...
    private toAssignable(context: Context, node: any): any {
        ...
        switch (node.type) {
            ...
            case 'AssignmentExpression':
                if (node.operator !== '=') this.tolerate(context, Errors.Unexpected);
    ...
    private parseYieldExpression(
        ...
        if (this.flags & Flags.ExtendedUnicodeEscape) this.tolerate(context, Errors.UnexpectedEscapedKeyword);
        if (context & Context.InParameter) this.tolerate(context, Errors.Unexpected);
    ...
    private parseAssignmentExpression(context: Context): any {
        ...
        if (this.token === Token.Arrow && (this.isIdentifier(context, t))) {
            if (t & Token.IsEvalArguments) {
                if (context & Context.Strict) this.report(Errors.InvalidBindingStrictMode, tokenDesc(t));
                this.flags |= Flags.ReservedWords;
            }
            return this.parseArrowFunctionExpression(context & ~Context.Async, pos, [expr]);
        }
        ...
        if (hasMask(this.token, Token.IsAssignOperator)) {
            ...
            if (context & Context.Strict && this.isEvalOrArguments((expr as ESTree.Identifier).name)) {
                ...
                // function body.
                if (context & Context.InParenthesis) this.flags |= Flags.SimpleParameterList;
    ...
    private parseConditionalExpression(context: Context, pos: Location) {
        ...
        if (this.parseOptional(context, Token.QuestionMark)) {
            ...
            this.expect(context, Token.Colon);
            if (context & Context.InClass && this.token & Token.IsEvalArguments) this.report(Errors.UnexpectedStrictReserved);
    ...
    private parseUnaryExpression(context: Context): ESTree.UnaryExpression | ESTree.Expression {
        const pos = this.getLocation();
        let t = this.token;
        if (t & Token.IsAwait && context & Context.Async) {
    ...
    private parseUpdateExpression(context: Context, pos: Location): any {
        ...
        if (hasMask(this.token, Token.IsUpdateOperator)) {
            ...
            this.nextToken(context);
            const t = this.token;
            if (context & Context.Yield && t & Token.IsAwait) this.tolerate(context, Errors.UnexpectedToken, tokenDesc(t));
            ...
            if (context & Context.Strict && this.isEvalOrArguments((argument as ESTree.Identifier).name)) {
                this.report(Errors.StrictLHSPrefixPostFix, 'Prefix');
            } else if (!isValidSimpleAssignmentTarget(argument)) {
                this.tolerate(context, Errors.InvalidLhsInPrefixPostFixOp, 'prefix');
            }
            return this.finishNode(context, pos, {
            ...
            if (!(this.flags & Flags.LineTerminator) && hasMask(this.token, Token.IsUpdateOperator)) {
                if (context & Context.Strict && this.isEvalOrArguments((expr as ESTree.Identifier).name)) {
                    this.tolerate(context, Errors.StrictLHSPrefixPostFix, 'Postfix');
                } else
                if (!isValidSimpleAssignmentTarget(expr)) {
                    this.tolerate(context, Errors.InvalidLhsInPrefixPostFixOp, 'postfix');
                }
                const operator = this.token;
                this.nextToken(context);
                return this.finishNode(context, pos, {
    ...
    private parseSuper(context: Context): ESTree.Expression {
        ...
        switch (this.token) {
            ...
            case Token.LeftParen:
                {
                    ...
                    if (!(context & Context.AllowConstructor)) {
                        this.tolerate(context, context & Context.Method ? Errors.UnexpectedSuper : Errors.BadSuperCall);
                    }
                    break;
                }
            ...
            case Token.Period:
                {
                    if (context & Context.InParameter && !(context & Context.Method)) {
    ...
    private parseImportCall(context: Context, pos: Location) {
        ...
        if (this.parseOptional(context, Token.Period)) {
            if (!(context & Context.Module)) this.tolerate(context, Errors.Unexpected);
            if (this.tokenValue !== 'meta') this.tolerate(context, Errors.Unexpected);
    ...
    private parseCallArguments(context: Context): ESTree.Expression[] {
        ...
        while (this.token !== Token.RightParen) {
            if (this.token === Token.Ellipsis) {
                ...
                if (this.token & Token.IsYield) this.flags |= Flags.Yield;
    ...
    private parsePrimaryExpression(context: Context, pos: Location): any {
        ...
        switch (this.token) {
            case Token.YieldKeyword:
                if (context & Context.Yield) {
                    ...
                    if (this.flags & Flags.ExtendedUnicodeEscape) {
                        this.report(Errors.UnexpectedEscapedKeyword);
                    }
                    //if (this.flags & Flags.InFunctionBody) this.tolerate(context, Errors.DisallowedInContext, tokenDesc(this.token));
                }
            case Token.AwaitKeyword:
                if (context & Context.Module) {
                    this.tolerate(context, Errors.UnexpectedToken, tokenDesc(this.token));
            ...
            case Token.FunctionKeyword:
                return this.parseFunction(context & ~(Context.Yield | Context.AnnexB) | Context.Expression);
            ...
            default:
                if (!this.isIdentifier(context, this.token)) this.report(Errors.Unexpected);
                return this.parseIdentifier(context);
        }
    }
    ...
    private parseLet(context: Context): ESTree.Identifier {
        ...
        if (context & Context.Strict) this.tolerate(context, Errors.InvalidStrictExpPostion, tokenDesc(t));
    ...
    private parseAsyncFunctionExpression(
        ...
        if (this.flags & Flags.LineTerminator) this.report(Errors.LineBreakAfterAsync);
        ...
        if (this.token === Token.FunctionKeyword) {
            if (hasEscape) this.report(Errors.UnexpectedEscapedKeyword);
            return this.parseFunction(
                context & ~Context.AnnexB | Context.Expression | Context.Async | Context.AsyncFunction,
                ObjectState.None,
                pos);
        }
        ...
        if (t & (Token.IsIdentifier | Token.IsYield)) {
            ...
            if (context & Context.Yield && t & Token.IsYield) this.report(Errors.Unexpected);
        ...
        while (this.token !== Token.RightParen) {
            ...
            if (this.token === Token.Ellipsis) {
                ...
                if (this.token & Token.IsYield) state |= ParenthesizedState.Yield;
        ...
        if (this.token === Token.Arrow) {
            ...
            if (state & ParenthesizedState.EvalOrArguments) {
                this.tolerate(context, Errors.UnexpectedStrictReserved);
            }
            ...
            return this.parseArrowFunctionExpression(context | Context.Async, pos, args, params);
        }
    ...
    private parseObjectElement(context: Context): ESTree.Property {
        ...
        if (!(state & ObjectState.Generator) && this.token === Token.AsyncKeyword) {
            ...
            if (this.token & Token.IsShorthand) {
                ...
                // Invalid: '({ \\u0061sync* m(){} });'
                if (isEscaped) this.report(Errors.UnexpectedEscapedKeyword);
        ...
        if (!(state & ObjectState.Computed) &&
            ...
            if (isEscaped) this.report(Errors.Unexpected);
    ...
    private parseArrayInitializer(context: Context): ESTree.ArrayExpression {
        ...
        if (state & ArrayState.CommaSeparator) {
            ...
            if (this.token === Token.Assign) this.report(Errors.ElementAfterRest);
    ...
    private parseClass(context: Context, optional = false): ESTree.ClassExpression | ESTree.ClassDeclaration {
        ...
        if (!id && !(context & Context.Expression) && !optional) this.tolerate(context, Errors.Unexpected);
    ...
    private parseClassElement(context: Context, state: ObjectState): any {
        ...
        if (!(this.token & Token.IsShorthand)) {
            ...
            if (t & Token.IsAsync && !(state & ObjectState.Computed) && !(state & ObjectState.Generator)) {
                ...
                if (this.flags & Flags.LineTerminator) this.tolerate(context, Errors.Unexpected);
        ...
        if (context & Context.OptionsNext) {
            if (this.token & (Token.IsIdentifier | Token.IsKeyword) ||
                ...
                if (state & ObjectState.Static) this.report(Errors.Unexpected);
                if (this.parseOptional(context, Token.Assign)) {
                    if (this.token & Token.IsEvalArguments) this.report(Errors.UnexpectedStrictReserved);
                    value = this.parseAssignmentExpression(context);
                }
    ...
    private parsePrivateName(context: Context, state: ObjectState = ObjectState.None) {
        ...
        const t = this.token;
        if (t === Token.ConstructorKeyword) this.tolerate(context, Errors.Unexpected);
        const name = this.tokenValue;
        this.nextToken(context);
        return this.finishNode(context, pos, {
            type: 'PrivateName',
    ...
    private parseArrowFunctionExpression(
        ...
        if (this.flags & Flags.HasCommaSeparator) this.report(Errors.ElementAfterRest);
        ...
        if (this.token === Token.LeftBrace) {
            body = this.parseFunctionBody(context & ~(Context.InParenthesis | Context.Yield) | Context.AllowIn | Context.ArrowFunction, params);
        } else {
            body = this.parseAssignmentExpression(context & ~(Context.InParenthesis | Context.Yield) | Context.AllowIn | Context.ArrowFunction);
            expression = true;
        }
        return this.finishNode(context, pos, {
    ...
    private parseParenthesizedExpression(context: Context): any {
        ...
        if (this.parseOptional(context, Token.RightParen) && this.token === Token.Arrow) {
            return this.parseArrowFunctionExpression(context & ~(Context.Async | Context.Yield), pos, []);
        }
        ...
        if (this.token === Token.Ellipsis) {
            ...
            this.expect(context, Token.RightParen);
            return this.parseArrowFunctionExpression(context & ~(Context.Async | Context.Yield), pos, [expr], params);
        }
        ...
        if (this.token & Token.IsBindingPattern) {
        ...
        if (this.token & Token.FutureReserved) {
        ...
        if (this.token === Token.Comma) {
            ...
            while (this.parseOptional(context, Token.Comma)) {
                ...
                if (this.parseOptional(context, Token.RightParen)) {
                    if (this.token === Token.Arrow)
                        return this.parseArrowFunctionExpression(context & ~(Context.Async | Context.Yield), pos, expressions, params);
                } else if (this.token === Token.Ellipsis) {
        ...
        if (this.token === Token.Arrow) {
            ...
            if (state & ParenthesizedState.EvalOrArguments) {
                ...
                if (context & Context.Strict) this.tolerate(context, Errors.UnexpectedStrictReserved);
                // Invalid: '(eval = 10) => { "use strict"; }'
                this.flags |= Flags.ReservedWords;
            }
            return this.parseArrowFunctionExpression(context & ~(Context.Async | Context.Yield), pos, isSequence ? (expr as any).expressions : [expr], params);
        }
    ...
    private parseBindingIdentifierOrPattern(context: Context, params: any[] = []) {
        ...
        if (t & (Token.IsAwait | Token.IsYield)) {
            if (t & Token.IsAwait && (context & (Context.Async | Context.Module))) {
                this.tolerate(context, Errors.UnexpectedReservedWord);
            } else if (t & Token.IsYield && (context & (Context.Yield | Context.Strict))) {
                this.tolerate(context, Errors.DisallowedInContext, tokenDesc(this.token));
            }
        }
        if (!(t & Token.IsBindingPattern)) {
    ...
    private parseAssignmentProperty(context: Context, params: any[] = []): ESTree.AssignmentProperty {
        ...
        if (t & (Token.IsIdentifier | Token.Keyword)) {
            ...
            if (state & ObjectState.Shorthand && context & Context.Yield && t & Token.IsYield) {
                this.tolerate(context, Errors.DisallowedInContext, tokenDesc(t));
            }
    ...
    private parseFunction(
        ...
        if (!(context & Context.Method)) {
            ...
            if (this.token !== Token.LeftParen) {
                ...
                if (context & (Context.Expression | Context.AnnexB)) {
                    ...
                    if ((context & Context.Async && t & Token.IsAwait) ||
                    ...
                    if ((prevContext & (Context.Async | Context.Module) && t & Token.IsAwait) ||
                        (prevContext & Context.Yield && t & Token.IsYield)) {
                        this.tolerate(context, Errors.DisallowedInContext, tokenDesc(t));
                    }
                }

                id = !(context & (Context.Strict | Context.Yield)) && this.token === Token.YieldKeyword ?
        ...
        const formalParameters = this.parseParameterList(context & ~Context.Method | Context.Expression | Context.InParameter, state);

        const args = formalParameters.args;
        const params = formalParameters.params;
        const body = this.parseFunctionBody(context & ~(Context.AnnexB | Context.Expression | Context.Method | Context.TopLevel), args);
    ...
    private parseFunctionBody(context: Context, params: any[] = []): any {
        ...
        if (this.token !== Token.RightBrace) {
            ...
            while (this.token === Token.StringLiteral) {
                ...
                if (context & Context.Strict || !isPrologueDirective(item)) break;

                if (hasStrictDirective) {
                    if (context & Context.StrictReserved) this.tolerate(context, Errors.UnexpectedStrictReserved);
    ...
    private parseParameterList(context: Context, state: ObjectState): any {
        ...
        while (this.token !== Token.RightParen) {
            ...
            if (state & ObjectState.HasRest) this.tolerate(context, Errors.InvalidParameterAfterRest);

            if (this.token === Token.Ellipsis) {

                if (state & ObjectState.Set) this.tolerate(context, Errors.BadSetterRestParameter);
                ...
                if (this.token === Token.Comma) this.report(Errors.ParamAfterRest);

                if (this.token === Token.Assign) this.report(Errors.InitializerAfterRest);
                ...
                if (this.parseOptional(context, Token.Assign)) {
                    ...
                    if (this.token & (Token.IsYield | Token.IsAwait) && context & (Context.Yield | Context.Async)) {
                        this.tolerate(context, Errors.DisallowedInContext, tokenDesc(this.token));
                    }
    ...
    private parseForStatement(context: Context): ESTree.ForStatement | ESTree.ForInStatement | ESTree.ForOfStatement {
        ...
        const awaitToken = !!(context & Context.Async) && this.parseOptional(context, Token.AwaitKeyword);
            ...
            if (t & Token.IsVarDecl) {
                switch (t) {
                    case Token.LetKeyword:
                        ...
                        // falls through
                    case Token.ConstKeyword:
                        context |= Context.Lexical | Context.Const;
                    default: // ignore
                }
