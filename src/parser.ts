...
export class Parser {
        ...
        private scanNumber(context: Context, ch: number): Token {
            ...
            if (ch === Chars.Zero) {
                ...
                if (state & NumericState.ContainsFragment) {
            ...
            if (this.flags & Flags.OptionsNext && this.nextChar() === Chars.LowerN) {
    
                // E.g. Invalid MV number - '2017.8n;'
                if (state & (NumericState.DecimalWithLeadingZero | NumericState.ImplicitOctal | NumericState.Float)) {
                    this.error(Errors.Unexpected);
                }
            ...
            if (state & (NumericState.ImplicitOctal | NumericState.DecimalWithLeadingZero)) {
                this.flags |= Flags.Octal;
            }
    
            if (this.flags & Flags.OptionsNext && !(state & NumericState.ContainsFragment) && this.flags & Flags.ContainsSeparator) {
            ...
            } else if (!(this.flags & Flags.OptionsNext)) {
                ...
                if (!value || !(state & (NumericState.ImplicitOctal | NumericState.Boh))) this.tokenValue = parseFloat(this.source.substring(start, end));
            } else if (!value) {
        ...
        private reinterpretAsPattern(context: Context, params: any) {
            ...
            switch (params.type) {
                ...
                case 'RestElement':
                    ...
                    if (params.argument.type === 'AssignmentPattern') this.error(Errors.InvalidRestDefaultValue);
                    return;
