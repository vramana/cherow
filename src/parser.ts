...
export class Parser {
    ...
    private parseFormalParameterList(context: Context, state = ObjectState.None): any {
        ...
        while (this.token !== Token.RightParen) {
            ...
            if (this.token === Token.Ellipsis) {
                ...
                if (this.token === Token.Comma) this.tolerate(context, Errors.ParamAfterRest);

                if (this.token === Token.Assign) this.tolerate(context, Errors.InitializerAfterRest);
    ...
    private parseJSXExpressionAttribute(
        context: Context
    ): ESTree.JSXExpressionContainer | ESTree.JSXSpreadChild {
    ...
    private parseJSXMemberExpression(
        context: Context,
