...
export class Parser {
    ...
    private readNext1(ch: Chars): number {
        this.advance();
    ...
    private scanString(context: Context, quote: number): Token {
        ...
        while (ch !== quote) {
            switch (ch) {
                ...
                case Chars.Backslash:
                    ch = this.readNext1(ch);

                    if (ch >= 128) {
                        ret += fromCodePoint(ch);
                    } else {
                        this.lastChar = ch;
                        const code = this.parseEscape(context, ch);

                        if (code >= 0) ret += fromCodePoint(code);
                        else this.handleStringError(code as Escape);
                        this.flags |= Flags.HasUnicode;
                        ch = this.lastChar;
                    }
                    break;
    ...
    private parseEscape(context: Context, first: number): number {
        ...
        switch (first) {
            ...
            case Chars.LowerX:
                {
                    const ch1 = this.lastChar = this.readNext();
                    const hi = toHex(ch1);
                    if (hi < 0) return Escape.InvalidHex;
                    const ch2 = this.lastChar = this.readNext();
                    const lo = toHex(ch2);
            ...
            case Chars.LowerU:
                {
                    let ch = this.lastChar = this.readNext1(first);
                    if (ch === Chars.LeftBrace) {
                        // \u{N}
                        // The first digit is required, so handle it *out* of the loop.
                        ch = this.lastChar = this.readNext1(ch);
                        let code = toHex(ch);
                    ...
                    case Chars.Backslash:
                        ...
                        if (ch >= 128) {
                            ...
                            if (code >= 0) {
                                ...
                                ret = null;
                                ch = this.scanBadTemplate(this.lastChar);
                                if (ch < 0) {
                                    ch = -ch;
                                    tail = false;
                                }
    ...
    private scanBadTemplate(ch: number): number {
        while (ch !== Chars.Backtick) {
                ...
                case Token.TemplateCont:
                    {
                        ...
                        const quasi = this.parseTemplate(context | Context.TaggedTemplate, this.getLocations());
                        expr = this.parseTaggedTemplateExpression(context | Context.TaggedTemplate, expr, quasi, this.getLocations());
                        break;
                    }
                case Token.TemplateTail:
                    {

                        const quasi = this.parseTemplateTail(context | Context.TaggedTemplate, this.getLocations());
                        expr = this.parseTaggedTemplateExpression(context | Context.TaggedTemplate, expr, quasi, pos);
                        break;
                    }
                default:
                    return expr;
    ...
    private parseTemplateTail(context: Context, pos: Location): ESTree.TemplateLiteral {
        ...
        const quasis = this.parseTemplateElement(context);
        return this.finishNode(pos, {
    ...
    private parseIdentifier(context: Context): ESTree.Identifier {
        if (context & Context.Strict && !(context & Context.Method) && this.token === Token.YieldKeyword) {
            this.throwUnexpectedToken();
        }
        const name = this.tokenValue;
        const pos = this.getLocations();
