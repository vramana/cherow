...
const files = {
  ...
  aurelia: fs.readFileSync(path.join(node_modules, '@aurelia', 'jit-html-browser', 'dist', 'index.iife.full.js')).toString(),
};
...
const onComplete = function onComplete(): void {
  ...
  for (let i = 0; i < results[1].length; ++i) {
    ...
    console.log(`${v1.name.padEnd(32)} ${format(v1.hz)} ops/sec${' '.repeat(8)} ${format(v2.hz)} ops/sec${' '.repeat(8)}${diffText}`);
  }
};
