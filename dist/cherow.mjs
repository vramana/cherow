...
Parser.prototype.peekExtendedUnicodeEscape = function peekExtendedUnicodeEscape () {
    ...
    // '\u{DDDDDDDD}'
    if (ch === 123 /* LeftBrace */) {
        ch = this.scanNext(ch, 55 /* InvalidHexEscapeSequence */);
        ...
        while (ch !== 125 /* RightBrace */) {
            ...
            ch = this$1.scanNext(ch, 55 /* InvalidHexEscapeSequence */);
        }
        return code;
    }
...
Parser.prototype.scanOctalDigits = function scanOctalDigits (context) {
    ...
    if (ch < 48 /* Zero */ || ch >= 56 /* Eight */)
        { this.error(42 /* InvalidBinaryDigit */); }
    this.advance();
    while (this.hasNext()) {
        ch = this$1.nextChar();
    ...
    switch (cp) {
        ...
        case 101 /* LowerE */:
            ...
            if (!(cp >= 48 /* Zero */ && cp <= 57 /* Nine */))
                { this.error(91 /* UnexpectedMantissa */); }
            this.advanceAndSkipDigits();
            end = this.index;
            break;
...
Parser.prototype.scanTemplate = function scanTemplate (context, first) {
    ...
    var tail = true;
    var ret = '';
    var ch = this.scanNext(first, 75 /* UnterminatedTemplate */);
    loop: while (ch !== 96 /* Backtick */) {
        switch (ch) {
            ...
            // '/'
            case 92 /* Backslash */:
                ch = this$1.scanNext(ch, 75 /* UnterminatedTemplate */);
                if (ch >= 128) {
                    ret += fromCodePoint(ch);
                }
                else {
...
Parser.prototype.isIdentifier = function isIdentifier (context, t) {
    if (context & 2 /* Strict */) {
        if (t === 8671304 /* LetKeyword */)
            { this.error(85 /* InvalidBindingStrictMode */, tokenDesc(this.token)); }
        if (context & 1 /* Module */) {
    ...
    switch (params.type) {
        ...
        case 'SpreadElement':
            ...
            if (context & 524288 /* ForStatement */ && params.argument.type === 'AssignmentExpression') {
                this.error(33 /* InvalidLHSInForIn */);
            }
        // Fall through
        case 'RestElement':
        ...
        switch (this$1.token) {
            case 16797801 /* StaticKeyword */:
                ...
                if (state & 2 /* Async */)
                    { break loop; }
                if (isEscaped)
                    { this$1.error(64 /* UnexpectedEscapedKeyword */); }
...
Parser.prototype.parseParenthesizedExpression = function parseParenthesizedExpression (context) {
    ...
    if (this.token === 10 /* Arrow */) {
        if (this.flags & 131072 /* Operator */)
            { this.error(104 /* IllegalArrowFuncParamList */); }
